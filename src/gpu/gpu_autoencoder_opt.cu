    ...
    int n, int width, int height, int depth, float * __restrict__ grad_in)
{
    ...
    if (base_in[idx00] == out_val)      atomicAdd(&base_grad_in[idx00], go);
    else if (base_in[idx01] == out_val) atomicAdd(&base_grad_in[idx01], go);
    else if (base_in[idx10] == out_val) atomicAdd(&base_grad_in[idx10], go);
    else                                atomicAdd(&base_grad_in[idx11], go);
}
    ...
    : batch_size(batch_size_), d_memory_pool(nullptr)
{
    ...
    printf("[Optim] Allocating Memory Pool: %.2f MB... ", this->total_memory_size / (1024.0 * 1024.0));
    ...
    // 3. Map Pointers
    size_t offset = 0;
    d_W1 = allocate_from_pool(d_memory_pool, offset, W1_s); d_b1 = allocate_from_pool(d_memory_pool, offset, 256);
    d_W2 = allocate_from_pool(d_memory_pool, offset, W2_s); d_b2 = allocate_from_pool(d_memory_pool, offset, 128);
    d_W3 = allocate_from_pool(d_memory_pool, offset, W3_s); d_b3 = allocate_from_pool(d_memory_pool, offset, 128);
    d_W4 = allocate_from_pool(d_memory_pool, offset, W4_s); d_b4 = allocate_from_pool(d_memory_pool, offset, 256);
    d_W5 = allocate_from_pool(d_memory_pool, offset, W5_s); d_b5 = allocate_from_pool(d_memory_pool, offset, 3);
    ...
    d_dW1 = allocate_from_pool(d_memory_pool, offset, W1_s); d_db1 = allocate_from_pool(d_memory_pool, offset, 256);
    d_dW2 = allocate_from_pool(d_memory_pool, offset, W2_s); d_db2 = allocate_from_pool(d_memory_pool, offset, 128);
    d_dW3 = allocate_from_pool(d_memory_pool, offset, W3_s); d_db3 = allocate_from_pool(d_memory_pool, offset, 128);
    d_dW4 = allocate_from_pool(d_memory_pool, offset, W4_s); d_db4 = allocate_from_pool(d_memory_pool, offset, 256);
    d_dW5 = allocate_from_pool(d_memory_pool, offset, W5_s); d_db5 = allocate_from_pool(d_memory_pool, offset, 3);
...
Gpu_Autoencoder_Opt::~Gpu_Autoencoder_Opt() {
    if (d_memory_pool) CHECK(cudaFree(d_memory_pool));
    delete[] recon_host;
}
...
void Gpu_Autoencoder_Opt::init_weights(int seed) {
    ...
    auto init_arr = [&](std::vector<float> &w, float s) { for(auto& v:w) v=rand_uniform(rng, -s, s); };
    init_arr(h_W1, 0.05f); init_arr(h_W2, 0.05f); init_arr(h_W3, 0.05f); init_arr(h_W4, 0.05f); init_arr(h_W5, 0.05f);
...
void Gpu_Autoencoder_Opt::forward(const float *input, int n, int width, int height, int depth) {
    ...
    CHECK(cudaMemcpy(recon_host, d_recon, n * 32 * 32 * 3 * sizeof(float), cudaMemcpyDeviceToHost));
}

void Gpu_Autoencoder_Opt::backward(const float *input, const float *target, int n, int width, int height, int depth) {
    int W1_s = 256 * 3 * 3 * 3; int W2_s = 128 * 3 * 3 * 256; int W3_s = 128 * 3 * 3 * 128;
    int W4_s = 256 * 3 * 3 * 128; int W5_s = 3 * 3 * 3 * 256;

    // Reset Gradients
    CHECK(cudaMemset(d_dW1, 0, W1_s * sizeof(float))); CHECK(cudaMemset(d_db1, 0, 256 * sizeof(float)));
    CHECK(cudaMemset(d_dW2, 0, W2_s * sizeof(float))); CHECK(cudaMemset(d_db2, 0, 128 * sizeof(float)));
    CHECK(cudaMemset(d_dW3, 0, W3_s * sizeof(float))); CHECK(cudaMemset(d_db3, 0, 128 * sizeof(float)));
    CHECK(cudaMemset(d_dW4, 0, W4_s * sizeof(float))); CHECK(cudaMemset(d_db4, 0, 256 * sizeof(float)));
    CHECK(cudaMemset(d_dW5, 0, W5_s * sizeof(float))); CHECK(cudaMemset(d_db5, 0, 3 * sizeof(float)));
...
void Gpu_Autoencoder_Opt::fit(const Dataset &dataset, int n_epoch, int batch_size_, float learning_rate, int seed, int checkpoint, const char *output_dir) {
    for (int epoch = 1; epoch <= n_epoch; ++epoch) {
        ...
        for (int b = 0; b < num_batches; ++b) {
            ...
            float loss = cpu_mse_loss(const_cast<float *>(x), recon_host, bn, 32, 32, 3);
            epoch_loss += loss;
